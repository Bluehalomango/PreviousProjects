...
public class NoiseSensor extends TimedSensor
     ...
     * current loudness reading.
     * Finds the rounded percentage complement of relative calculated loudness.
     *
     * @return level of comfort at sensor location, 0 to 100
     */
    @Override
    public int getComfortLevel() {
        //Calculates the comfort level using the given info
        float comfortLevel = (float) (1-calculateRelativeLoudness()) * 100;
        //If comfort level is less than 0, 0 is returned
        if (comfortLevel < 0) {
            return 0;
        } else {
            //Rounds the comfort level down to its integer floor
            return (int) Math.floor(comfortLevel);
        }
    }
