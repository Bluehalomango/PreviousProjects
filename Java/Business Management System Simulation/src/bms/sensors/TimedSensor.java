...
public abstract class TimedSensor implements TimedItem, Sensor, Encodable {
    ...
    public String encode() {
        ...
        StringBuilder encodedTimedSensor = new StringBuilder();
        encodedTimedSensor.append(sensorReadings[0]);
        //Goes through and appends the sensor readings to the builder as csv
        for (int i = 1; i < sensorReadings.length; i++) {
            encodedTimedSensor.append(',');
            encodedTimedSensor.append(sensorReadings[i]);
        }
        return encodedTimedSensor.toString();
    }

    /**
     * Returns true if and only if this timed sensor is equal to the other
     * given sensor. For two timed sensors to be equal, they must:
     *
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof  TimedSensor) {
            //Casts given object to a TimedSensor class
            TimedSensor sensorCompare = (TimedSensor) obj;
            //Checks that they share the update frequency and reading length
            if (sensorCompare.getUpdateFrequency() == this.getUpdateFrequency()
                    && sensorCompare.sensorReadings.length ==
                    this.sensorReadings.length) {
                //Goes through each sensor reading and compares them
                for (int i = 0; i < this.sensorReadings.length; i++) {
                    if (sensorCompare.sensorReadings[i]
    ...
    /**
     * Returns the uniquely generated hash code of this timed sensor
     * Makes use of the update frequency, sensor reading, values and class name.
     * @return hash code of this sensor
     */
    @Override
    public int hashCode() {
        int hash = 17;
        hash = 43 * hash + this.updateFrequency;
        hash = 43 * hash + this.sensorReadings.length;
        for (Integer reading: this.sensorReadings) {
            hash = 43 * hash + reading;
        }
        hash = 43 * hash + (this.getClass() == null ? 0 :
