...
public class ViewModel {
    ...
    public void accept(KeyCode keyCode) {
        //Checks if code input is for toggle pause
        if (keyCode == KeyCode.P) {
            this.togglePause();
        } else if (keyCode == KeyCode.S) {
            //Checks if the user wishes to quicksave
            try {
    ...
    public void togglePause() {
        //Sets pause value to inverse
        this.paused.setValue(!this.paused.get());
        String pauseText;
        //Checks to determine string text to set button with
        if (this.paused.get()) {
            pauseText = "Unpause";
        } else {
            pauseText = "Pause";
        }
        pauseButtonText.setValue(pauseText);
    }
    ...
    public void save(String filename) throws IOException {
        //Creates a new FileWriter and StringBuilder to save the info
        FileWriter savedFile = new FileWriter(new File(filename));
        StringBuilder saveInfo = new StringBuilder();
        saveInfo.append(buildings.get(0).encode());
        //Goes through all the buildings and appends each one's encode return
        for (int i = 1; i < this.buildings.size(); i++) {;
            saveInfo.append(System.lineSeparator());
            saveInfo.append(buildings.get(i).encode());
        }
        //Writes StringBuilder to the file and closes the file
        savedFile.write(String.valueOf(saveInfo));
        savedFile.close();
    }
    ...
    public void tick() {
        //Checks if paused
        if (!this.paused.get()) {
            //Runs the elapseOneMinute and increases the tick value
            TimedItemManager.getInstance().elapseOneMinute();
            ticks.setValue(this.ticks.getValue() + 1);
